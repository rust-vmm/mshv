...
impl VmFd {
    ...
    pub fn hvcall_install_intercept(&self, access_type_mask: u32, intercept_type: u32, intercept_param: hv_intercept_parameters) -> Result<()> {
        let input = hv_input_install_intercept {
            access_type_mask: install_intercept_args.access_type_mask,
    ...
    pub fn hvcall_register_deliverability_notifications(&self, vp: u32, flag: u64) -> Result<()> {
        self.hvcall_set_reg(vp, &[hv_register_assoc {
            reg: hv_register::HV_REGISTER_DELIVERABILITY_NOTIFICATIONS,
            value: unsafe { hv_register_value: { reg64: flag } },
        }])
    }
    ...
    pub fn hvcall_get_partition_property(&self, code: u32) -> Result<u64> {
        ...
        let mut output = hv_output_set_partition_property {
            ..Default::default()
        };
