...
// The kernel `struct mshv_signal_mask` has `__u32 len` + trailing flex
// array; its `sizeof` is 4, so `u32` produces the matching ioctl number.
ioctl_iow_nr!(MSHV_SET_SIGNAL_MASK, MSHV_IOCTL, 0x03, u32);
